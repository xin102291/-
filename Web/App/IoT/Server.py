...
from App.route import socketio
q = 999999937
n = 3
scale = 10000
bound = 5
...
def message_handle(client,info):
    ...
    socketio.emit('encryption_step', {'step': f'驗證訊息(server_m): {m}'})
            ...
            if "CONNECT" == cmd:
                ...
                try:
                    ...
                    for i in range(5):
                        ...
                        count += 1
                    print(session_key)
                    socketio.emit('encryption_step', {'step': f'session_key: {session_key}'})
                    # Encrypt m array and store in c array
                    c = [E.encrypt(num, pk_iot) for num in m]
                    # print("Encrypted messages:", c)
                    socketio.emit('encryption_step', {'step': f'Encrypted messages: {c}'})
                    c = json.dumps(c)
                ...
                data = jd['data']['data']
                print("接收訊息: ",data)
                socketio.emit('encryption_step', {'step': f'receive message: {data}'})
                # print(len(data))
                if len(data) == 2: #以長度判斷是否為正常驗證訊息
                    print("驗證簽章")
                    c = data[0]
                    p = E_iot.decrypt(c[0], pk_iot)
                    socketio.emit('encryption_step', {'step': f'p: {p}'})
                    socketio.emit('encryption_step', {'step': '驗證簽章(p == m)'})
                    if(p == m):
                        print("驗證成功")
                        socketio.emit('encryption_step', {'step': '驗證成功'})
                        print("產生簽章:")
                        c = E_server.encrypt(data[1][0], pk_server, sk_server)
                        socketio.emit('encryption_step', {'step': f'產生簽章: {c}'})
                        print(c)
                        c= [c]
                        c = json.dumps(c)
                        client.sendall(c.encode(encoding="utf8"))
...
def remove_client(client_name):
    ...
    if None != client:
        ...
        socketio.emit('encryption_step', {'step': f'IoT: {client_name}'})
